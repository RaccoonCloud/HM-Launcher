...
# Current + legacy EXE names (older releases were branded HarbourMaster)
_EXE_NAMES = (f"{APP_NAME}.exe", "ShipYard.exe", "HarbourMaster.exe")
...
def apply_launcher_update(
    ...
    *,
    ...
    if not getattr(sys, "frozen", False):
        raise RuntimeError(f"Launcher self-update only works from the built {APP_NAME}.exe")
    ...
    if new_exe is None or not new_exe.is_file():
        raise RuntimeError(f"Update package did not contain {APP_NAME}.exe")

    pending = app_dir / f"{current_exe.name}.new"
